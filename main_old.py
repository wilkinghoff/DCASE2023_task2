...
def model_emb_cnn(num_classes, raw_dim, n_subclusters, use_bias=False):
    ...
    x_mix = x
    x_mix, y = MixupLayer(prob=1)([x_mix, y])
    ...
    #x = tf.keras.layers.Multiply()([emb_fft, emb_mel])
    #x = tf.keras.layers.Dense(128, kernel_regularizer=l2_weight_decay, use_bias=use_bias)(x)
...
for k_ensemble in np.arange(ensemble_size):
    ...
    for k in np.arange(aeons):
        ...
        print('aeon: ' + str(k+1))
        # fit model
        weight_path = 'wts_' + str(k+1) + 'k_' + str(target_sr) + '_' + str(k_ensemble+1) + '_old_no-bias.h5'
...
sub_path = './teams/submission/team_fkie_old'
...
for j, cat in enumerate(np.unique(test_ids)):
    ...
    results_dec.to_csv(sub_path + '/decision_result_' + cat.split('_')[0] + '_section_' + cat.split('_')[-1] + '_test.csv',
                       encoding='utf-8', index=False, header=False)
np.save('pred_eval_old_no-bias.npy', pred_eval)
np.save('pred_unknown_old_no-bias.npy', pred_unknown)
np.save('pred_train_old_no-bias.npy', pred_train)
np.save('pred_test_old_no-bias.npy', pred_test)
print('####################')
print('####################')
print('####################')
