...
from statistics_exchange_layer_fixed import XchangeLayer
...
def model_emb_cnn(num_classes, raw_dim, n_subclusters, use_bias=False):
    ...
    #x_mix, y = MixupLayer(prob=0.5)([x, y])
    x_mix = x

    # magnitude
    x_mag = tf.keras.layers.Reshape((raw_dim,))(x_mix)
    x_mag = MagnitudeSpectrogram(16000, 1024, 512, f_max=8000, f_min=200)(x_mag)
    #x_mag = tf.keras.layers.Lambda(lambda x: x - tf.math.reduce_mean(x, axis=1, keepdims=True))(x_mag) # CMN-like normalization
    #x_mag = tf.keras.layers.BatchNormalization(axis=-2)(x_mag)
    ...
    x_mag, x, y = XchangeLayer(prob=0.5, num_classes=num_classes)([x_mag, x, y])
    ...
    x = tf.keras.layers.Conv2D(16, 7, strides=2, activation='linear', padding='same',
                               kernel_regularizer=l2_weight_decay, use_bias=use_bias)(x_mag)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.ReLU()(x)
    ...
    output = SCAdaCos(n_classes=num_classes*2, n_subclusters=n_subclusters, trainable=True)([x, y, label_input])
        ...
        for label, category in enumerate(os.listdir(dict)):
            ...
            for count, file in tqdm(enumerate(os.listdir(dict + category + "/train")),
                ...
                if file.endswith('.wav'):
                    ...
                    #raw = librosa.util.pad_center(raw, size=288000)
...
pred_eval_simu = np.load('pred_eval.npy')
pred_unknown_simu = np.load('pred_unknown.npy')
for k_ensemble in np.arange(ensemble_size):
    ...
    for k in np.arange(aeons):
        ...
        print('aeon: ' + str(k+1))
        # fit model
        weight_path = 'wts_' + str(k+1) + 'k_' + str(target_sr) + '_' + str(k_ensemble+1) + '_stat_xchange_fixed.h5'
        if not os.path.isfile(weight_path):
            ...
            model = tf.keras.models.load_model(weight_path,
                                               custom_objects={'MixupLayer': MixupLayer, 'XchangeLayer': XchangeLayer, 'mixupLoss': mixupLoss,
                                                               'SCAdaCos': SCAdaCos,
        ...
        for j, lab in tqdm(enumerate(np.unique(train_labels))):
            ...
            for k_cluster in tqdm(np.unique(cluster_labels)):
                cov = np.cov(x_train_ln[(train_labels == lab)][cluster_labels == k_cluster].transpose())
                icov = np.linalg.inv(cov+np.eye(cov.shape[0])*1e-2)
                mu = np.mean(x_train_ln[(train_labels == lab)][cluster_labels == k_cluster], axis=0, keepdims=True)
                ...
                #plt.plot(maha_eval[:,k_cluster])
                #plt.plot(maha_unknown[:,k_cluster])
                #plt.show()
        ...
        for j, cat in enumerate(np.unique(test_ids)):
            ...
            results_dec.to_csv(sub_path + '/decision_result_' + cat.split('_')[0] + '_section_' + cat.split('_')[-1] + '_test.csv',
                               encoding='utf-8', index=False, header=False)
        """
pred_eval += pred_eval_simu
pred_unknown += pred_unknown_simu
